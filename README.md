...
Ход работы:
- Я повнимательнее изучила rollerball_config.yaml файл в 1 проекте и поменяла некоторые переменные, и вот к какому выводу пришла:
- Изменение batch_size до 12 приводит к немного более быстрому обучению, так как это количество объектов, которые могут обрабатываться одновременно для обучения одной модели. Обобщённо - это ограничение памяти. Так как для завершения одной эпохи должно произойти сколько-то обновлений, то при маленьком значении batch_size, а соответственно и небольшом количестве параллельно обрабатываемых объектов, потребуется куда больше времени, чтобы достигло нужное количество обновлений и завершилась эпоха. Поэтому batch_size  влияет прямопропорционально на обучение модели.
- Изменение learning_rate - я уменьшила, еле дождалась хотя бы одного результата до 1.0е-5. Эта величина - скорость обучения, это некоторое пороговое значение, которое чем меньше, тем сложнее агенту пройти обучение и тем лучше и качественннее обучение, но с другой стороны, чем значение выше, тем и скорость обучения больше, но тем не качественнее обучение. Допустим, если высокое пороговое значение, то шарику достаточно лишь немного приблизиться к цели, даже не обязательно двигаться точно в направлении цели, чтобы получить вознаграждение. Соответственно эта переменная влияет прямопропорционально на скорость обучения, и обратнопропорционально качеству обучения.
- num_epoch - изменение количества эпох, я увеличила до 5 эпох, и это привело к увеличению времени ожидания обучения агента, но с другой стороны большее количество эпох - это наиболее качественное обучение. Значит, с увеличением количества эпох прямопропорционально увеличивается время ожидания обучения, и обратнопропорционально увеличению скорости обучения, но, главное, прямопропорционально качеству обучения.
- Результаты изменения обучения TensorBoard прикреплю ниже:
  
![Снимок экрана 2024-12-19 184910](https://github.com/user-attachments/assets/400741ab-c56e-4740-9b3f-b417127c8cf9)

![Снимок экрана 2024-12-19 184921](https://github.com/user-attachments/assets/c3d7fda0-e14a-4254-9fe2-febca17f0b75)

![Снимок экрана 2024-12-19 184928](https://github.com/user-attachments/assets/590062b1-48bf-4708-868e-86578fcdde2c)

![Снимок экрана 2024-12-19 184931](https://github.com/user-attachments/assets/a9f89428-5425-45e2-a707-e9f305116401)

## Задание 3
### Приведите примеры, для каких игровых задачи и ситуаций могут использоваться примеры 1 и 2 с ML-Agent’ом. В каких случаях проще использовать ML-агент, а не писать программную реализацию решения? 

Ход работы:
Примеры использования 1 и 2 ML-агентов (буду сразу писать и почему так проще, и как это можно связать с агентами):
- Например, если поставлена задача настроить противника под уровень игрока, делая игру увлекательной: не слишком легкой, не слишком сложной. А игроки бывают разные с разными уровнями подготовки к прохождению игры, взять тех же новичков и профессионалов, результаты будут как ни странно разные. Поэтому под всех не подстроить один единый уровень с одними и теми же изначальными условиями. Надо сначала пронаблюдать за откликом игрока на испытания, и уже потом подстраивать под него объект, или противника. Пример к этой задаче: есть игры-гонки, смысл которых обогнать всех NPC и добраться до финиша первым. Так, обучая NPC игре пользователя, NPC может составить реальную конкуренцию пользователю, который будет схожим образом передвигаться, допуская иногда ошибки и запинаясь, иногда нагоняя пользователя, чтобы тот максимально почувствовал ощущение реальных состязаний.
- Вспоминается игра в пинг-понг против NPC персонажа. Вот только я знала игру, где изначально были установленные разные уровни сложности, и на протяжении одной игры они не меняются. Но если учесть, что игрок может отточить свои навыки во время игры, то почему бы точно так же агенту не обучаться, анализируя каждый шаг пользователя и подстраиваясь под него, чтобы вся игра была похожа на игру против реального человека. Так обучение ML-агента, но не того же NPC персонажа, а грубо говоря шарика для пинг-понга, можно сделать один универсальный уровень для всех игроков, это сократило бы тонну кода для каждого нового уровня, в котором по сути меняются только некоторые величины, а всё остальное остаётся тем же.
- Допустим, есть много много противников у персонажа, который бегает по локациям и сражается. Простой пример будет, это обучить этих противников ходить не по определённой, изначально заданной траектории, а наоборот при приближении персонажа, самим к нему направляться с боем. Так, например, в игре Alien Isolations чужой как будто всегда знает, где находится персонаж и постоянно будет где-то в ближайшей локации или ходить всегда рядом. Допустим, если в данной игре персонаж один, то несложно прописать ему определённую траекторию, но если их много, то вместо того, чтобы копировать один и тот же код, лучше и проще обучить агента следовать за игроком и если игрок приблизится, то биться с ним.
## Выводы
